...
def get_datasets(logdir, condition=None):
    ...
    for root, _, files in os.walk(logdir):
        if 'progress.txt' in files:
            ...
            exp_data['EpMaxCost'] = exp_data.filter(regex='^EpMaxCost', axis=1).max(axis=1)
            ...
            if max_cost in exp_data:
                exp_data.insert(len(exp_data.columns),'Maximum_Statewise_Cost',exp_data[max_cost])
...
def make_plots(all_logdirs, legend=None, xaxis=None, values=None, count=False,  
               font_scale=1.5, smooth=1, select=None, exclude=None, estimator='mean', results_dir=None, title='reward', reward_flag=True, cost_flag=False, max_cost_flag=False):
...
def main():
    ...
    make_plots(args.logdir, args.legend, args.xaxis, args.value, args.count, 
               ...
               estimator=args.est, results_dir=args.results_dir, title=args.title, reward_flag=reward_flag, cost_flag=cost_flag, max_cost_flag = max_cost_flag)
